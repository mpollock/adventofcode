#2d list, 1000x1000, given input for on off by rectagle corner coordinates
#on mean turn up brightness, off means turn down, toggle is up by two
string = """turn off 660,55 through 986,197
...
for command in commands:
	if "off" in command:
		coordOne = command.split(" ")[2]
		coordTwo = command.split(" ")[4]
		onOff = -1
	elif "on" in command:
		coordOne = command.split(" ")[2]
		coordTwo = command.split(" ")[4]
		...
		coordOne = command.split(" ")[1]
		coordTwo = command.split(" ")[3]
		onOff = 2
	xStart = int(coordOne.split(",")[0])
	xStop = int(coordTwo.split(",")[0])+1
	yStart = int(coordOne.split(",")[1])
	yStop = int(coordTwo.split(",")[1])+1
